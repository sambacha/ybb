...
/// @title YBearBull Vault Helper contract
...
contract vaultHelper is Owned {
    ...
    /*
     * @notice Returns the buy bonus based on the incoming ETH and selected token. Only relevant to token buys
     * @param {token} The selected bull or bear token
     * @param {eth} The amount of ETH to be added
     * @return {Bonus in ETH} uint256
     */
    function getBonus(
        address vault,
        address token,
        ...
        if (balanceEquity > 0 && totaleth0 > tokeneth0.mul(2)) {
            ...
            // @dev If the after buy ratio is grater than .5 (50%) we reward the entire balance equity
            return (
                ratio1 <= 5 * 10**17
    ...
    /*
     * @notice Returns the sell penalty based on the outgoing ETH and selected token. Only relevant to token sells.
     * @param {token} The selected bull or bear token
     * @param {eth} The amount of outgoing ETH
     * @return {Penalty in ETH} uint256
     */
    function getPenalty(
        address vault,
        address token,
        ...
        // @dev Grab total equity of only target token
        uint256 tokeneth0 = ivault.getTokenEquity(token);
        // @dev Calc target token equity after sell
        uint256 tokeneth1 = tokeneth0.sub(eth);
        // @dev Only calc penalty if ratio is less than .5 (50%) after token sell
        if (totaleth0.div(2) >= tokeneth1) {
            // @dev Current ratio of token equity to total equity
            uint256 ratio0 = tokeneth0.mul(1 ether).div(totaleth0);
            // @dev Ratio of token equity to total equity after buy
    ...
    function getSharePrice(address vault) public view returns (uint256) {
        ...
        if (ivault.getTotalLiqShares() == 0) {
            ...
            return (
                ivault.getLiqEquity(bull).add(ivault.getLiqEquity(bear)).add(ivault.getLiqFees()).mul(1 ether).div(
                    ivault.getTotalLiqShares()
                )
            );
        }
    }
    ...
    /*
     * @notice Calculate how many bull/bear tokens virtually mint based on incoming ETH.
     * @returns {uint256, uint256, uint256, uint256} bull/bear equity and bull/bear tokens to be added
     */
        ...
        )
    {
        ...
        uint256 bullEquity = ivault.getLiqEquity(bull) < ivault.getLiqEquity(bear)
            ? ivault.getLiqEquity(bear).sub(ivault.getLiqEquity(bull))
            : 0;
        uint256 bearEquity = ivault.getLiqEquity(bear) < ivault.getLiqEquity(bull)
            ? ivault.getLiqEquity(bull).sub(ivault.getLiqEquity(bear))
            : 0;
    ...
    /*
     * @notice Calculate how many bull/bear tokens virtually burn based on shares being removed.
     * @param shares Amount of shares user removing from LP
     * @returns {uint256, uint256, uint256, uint256, uint256} bull/bear equity and bull/bear tokens to be removed
     */
        ...
        )
    {
        ...
        address bear = ivault.getBearToken();
        uint256 eth = shares
            .mul(ivault.getLiqEquity(bull).add(ivault.getLiqEquity(bear)).mul(1 ether).div(ivault.getTotalLiqShares()))
            .div(1 ether);
        uint256 bullEquity = ivault.getLiqEquity(bull) > ivault.getLiqEquity(bear)
            ? ivault.getLiqEquity(bull).sub(ivault.getLiqEquity(bear))
            : 0;
        ...
        bullTokens = bullTokens > ivault.getLiqTokens(bull) ? ivault.getLiqTokens(bull) : bullTokens;
        bearTokens = bearTokens > ivault.getLiqTokens(bear) ? ivault.getLiqTokens(bear) : bearTokens;
        ...
        return (bullEquity, bearEquity, bullTokens, bearTokens, feesPaid);
    }
